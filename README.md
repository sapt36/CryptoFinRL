        ...
        訓練機器學習模型交易加密貨幣投資組合 FinRL: Deep Reinforcement Learning to trade a cryptocurrency portfolio 
...
4. 過擬合問題與風險管理：加密貨幣市場可能具有高度非線性和隨機性，這可能導致模型過度擬合訓練數據，從而導致在未來表現不佳。需要使用技術來解決過擬合問題。我們使用的避險機制是將偵測到大盤(比特幣)大跌時，將所有加密貨幣換成穩定幣(BUSD)止損。

5. 數據切分：測試集與訓練集數據時間段的切分，是訓練出的模型是否可用且符合正當性的關鍵，最後比例為2:1，以下將會說明
## 歷史幣價資料取得
* [幣安API](https://binance-docs.github.io/apidocs/websocket_api/cn/#185368440e)
## 所使用幣種
| BTC  | ETH | BNB | SOL | BUSD |
|----|----|----|----|----|
<br>
BUSD （Binance USD）的穩定幣有兩種，分別是發行方為 Paxos、由以太坊於ERC-20機制鑄造而成的BUSD；<br>
以及由幣安透過幣安掛勾機制「Binance-Peg」／「BEP-20」、鎖定以太坊上BUSD而提供的同價值穩定幣，<br>
目的在於長期以1：1美元價格作為穩定化交易代幣。<br>

## 資料切分
* 訓練時間段 : 2020/10/17 ~ 2021/10/17
* 測試時間段 : 2021/10/18 ~ 2023/10/25
* 選擇從2020/10/17的資料開始做訓練，是因為對照資料中的大盤：Bitcoin，價格從那時才開始劇烈波動，而如果採用6：4的慣例來拆分訓練集與測試集，則會使資料因為近一年Bitcoin皆持續升值而沒有測試價值，因此最後由訓練集一年，測試集兩年來將數據做切分。
...
* [SuperTrend 指標](https://tw.tradingview.com/scripts/supertrend/)
* [Catch22 時間序列特徵](https://github.com/DynamicsAndNeuralSystems/pycatch22)
* 過去平均跌幅
* [微軟QLib 開高低收特徵](https://github.com/microsoft/qlib)
