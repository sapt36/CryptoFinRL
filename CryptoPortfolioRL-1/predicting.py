...
def get_ready_data(api_key, secret_key, freq, crypto_list, limit):
    ...
    for i in range(len(crypto_list)):
        ...
        bars = exchange.fetch_ohlcv(TICKER, timeframe=freq, limit=limit)
    ...
    close_data = pd.crosstab(index=ohlcv_df['date'], columns=ohlcv_df['ticker'], values=ohlcv_df['close'], aggfunc=lambda s:s)

    return ohlcv_df, close_data
...
def get_features(folder, crypto_list, feature_list, close_data, ohlcv):

    ohlcv['date'] = pd.to_datetime(ohlcv['date'], format="%Y-%m-%d %H:%M:%S")
    ohlcv = ohlcv.set_index('date')
    ohlcvs=[]
    for i in range(len(crypto_list)):
        temp = ohlcv.loc[ohlcv['ticker']==crypto_list[i]].resample('8h').ffill().reset_index()
        ohlcvs.append(temp)
    ...
    if 'hedging' in feature_list:
        ...
        returns_1w = close_data[MOMENTUM].resample(FREQ).ffill().pct_change()
        returns_3w = close_data[MOMENTUM].resample(FREQ).ffill().pct_change().add(1).rolling(3).apply(np.prod)
        returns_6w = close_data[MOMENTUM].resample(FREQ).ffill().pct_change().add(1).rolling(6).apply(np.prod)
        ...
        returns_mom = returns_mom.reindex(close_data[MOMENTUM].index, method='ffill').fillna(method='bfill').reset_index()
        returns_mom.columns = ['date', 'hedging']
    ...
    if 'dd' in feature_list:
        ...
        for i in range(len(crypto_list)):
            dd_df[crypto_list[i]] = drawdown(close_data[crypto_list[i]])
    ...
    if '86ta' in feature_list:
        ...
        for i in range(len(crypto_list)):
            temp = ohlcv.loc[ohlcv['ticker']==crypto_list[i]].reset_index(drop=True).drop(['ticker'], axis=1)
            ta_df = ta.add_all_ta_features(temp, open="open", high="high", low="low", close="close", volume="volume", fillna=True)
            ta_df['ticker'] = crypto_list[i]
    ...
    data = data.set_index('date')
    cols = ['ticker', 'close', 'open', 'low', 'high', 'hedging']
    data_temp = data[cols]

    mms = joblib.load(f'{folder}minmax_scaler.joblib')
    pt = joblib.load(f'{folder}power_transformer.joblib')

    data_scaled = mms.transform(data.drop(cols, axis=1).values)
    data_transformed = pt.transform(data_scaled)

    data_transformed = pd.DataFrame(data_transformed, index=data.index, columns=data.drop(cols, axis=1).columns)
    data_transformed = pd.concat([data_temp[cols[:-1]], data_transformed, data_temp[['hedging']]], axis=1).reset_index()

    # only keep last row
    data_transformed['date'] = data_transformed['date'].apply(lambda x:str(x))
    data_transformed.index = data_transformed.date.factorize()[0]
    data_transformed = data_transformed.loc[data_transformed.index.unique()[-1]]
    data_transformed.index = data_transformed.date.factorize()[0]

    return data_transformed, close_data
...
def get_predict_result(folder, agent_name, hedging, n, deterministic, tickers, rl_model_filename, trade):

    tech_indicator_list = trade.columns.tolist()[3:]
    ...
    model = MODELS[agent_name].load(f'{folder}{rl_model_filename}', custom_objects=custom_objects)

    new_data = trade.loc[trade.index.unique()[-n]]
    next_obs = np.array([new_data[tech].values.tolist() for tech in tech_indicator_list])
    next_obs = next_obs.astype('float64')
    action, _states = model.predict(next_obs, deterministic=deterministic)
    action = action.flatten()

    # normalized to around 0.5
    action_df = pd.DataFrame()
    action_df['raw_action'] = action
    action_df['scaled_action'] = (action_df['raw_action'].rank(method="first")-0.5)/len(action_df)
    action = action_df['scaled_action'].values

    weights = softmax_normalization(action)

    # mom <= 0 hedgeing
    if hedging[0]:
        if new_data['hedging'].values[0] <= 0:
            weights = np.array([0]*len(tickers))
            weights[hedging[1]] = 1

    weight_df = pd.DataFrame()
    weight_df['ticker'] = sorted(tickers)
    weight_df['weight'] = weights
    weight_df = weight_df.loc[weight_df['weight']!=0].reset_index(drop=True)
